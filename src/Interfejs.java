					...
					public void run() {
						if(table.getTableModel().getRowCount() == 1) {
						   ...
						   window.close();
							...
							{
								System.out.println("Tu sobie wyskoczyÅ‚ wyjÄ…tek, ale cicho XDDDDDDD");
							}
							window.close();
					}
				});
	...
	public void Dodaj_lekarza() {
		...
		Button button = new Button("Dodaj", new Runnable() {
			...
			public void run() {
				...
				{
					new MessageDialogBuilder().setTitle("Error").setText("Login i haslo musza mieÄ‡ co najmniej 6 znakÃ³w.")
					.addButton(MessageDialogButton.Close.valueOf("OK")).build().showDialog(gui);
					return;
				}
				if(Integer.parseInt(TextWiek.getText()) < 18 || Integer.parseInt(TextWiek.getText()) > 80)
				{
					new MessageDialogBuilder().setTitle("Error").setText("ZÅ‚y wiek.")
					.addButton(MessageDialogButton.Close.valueOf("OK")).build().showDialog(gui);
					return;
				}
				if(TextTelefon.getText().length() < 9)
				{
					new MessageDialogBuilder().setTitle("Error").setText("Telefon za krÃ³tki.")
					.addButton(MessageDialogButton.Close.valueOf("OK")).build().showDialog(gui);
					return;
				}
				for (Lekarz L : C.getLekarze()) {
					...
					if (TextLogin.getText().equals(L.getLogin())) {
						new MessageDialogBuilder().setTitle("Error").setText("Login juÅ¼ istnieje.")
								.addButton(MessageDialogButton.Close.valueOf("OK")).build().showDialog(gui);
						return;
					}
				}
		...
		Button button = new Button("Dodaj", new Runnable() {
			...
			public void run() {
					...
					try {
						...
						new MessageDialogBuilder().setTitle("Error").setText("Å¹le podana data.")
						.addButton(MessageDialogButton.Close).build().showDialog(gui);
						return;
					}
	...
	public void Okno_glowne() throws IOException {
		...
		Pacjent = new Button("Pacjenci", new Runnable() {
			...
			public void run() {
				...
				table = new Table<String>("Pesel","Imie","Nazwisko","Wiek", "Ulica", "Numer domu", "Numer mieszkania", "MiejscowoÅ›Ä‡");
		...
		mainPanel.addComponent(base1Panel1.withBorder(Borders.doubleLineReverseBevel("SkrÃ³ty")));
		//
		base1Panel1.addComponent(new EmptySpace(new TerminalSize(3, 0)));
		base1Panel1.addComponent(new Label("F6: Dodaj"));
		base1Panel1.addComponent(new EmptySpace(new TerminalSize(3, 0)));
